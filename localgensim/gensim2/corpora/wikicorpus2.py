...
#doc = nlp2('european union authorities fined Google as reporeted by the new york times on Wednesday the third.')
#[ent for ent in doc.ents if len(ent)>1]

from flashtext import KeywordProcessor
#keyword_processor = sv.load("keyword_processor_EM")
...
def tokenizeEM(content, token_min_len=TOKEN_MIN_LEN, token_max_len=TOKEN_MAX_LEN, lower=True):
...
def tokenizeDEP(content, token_min_len=TOKEN_MIN_LEN, token_max_len=TOKEN_MAX_LEN, lower=True):
    sentence =  [utils.to_unicode(token) for token in utils.tokenize(content, lower=lower, errors='ignore')
               if token_min_len <= len(token) <= token_max_len and not token.startswith('_')]
    sentence_ = filter(lambda i: False if i in '[] ' else True, sentence)
    holders = dict() #holding phrase marker locations
    for i,x in enumerate(sentence):
        if x in '[]':
            holders[i]=x
    sentence_ = list(sentence_) 
    sentence_ = nlp(" ".join(sentence_)) #parsing the sentence
    sentence_tagged = list()
    for word in sentence_:
        sentence_tagged.append(word.text+'#'+stemmer.stem(word.head.text))
    #Re-inserting markers
    for i,x in holders.items():
        sentence_tagged.insert(i,x)
    _sentence = list()
    i = 0
    while i<len(sentence_tagged):
        if sentence_tagged[i] == '[':
            _sentence.append(sentence_tagged[i])
            i+=1
            if i == len(sentence_tagged):
                break
            while sentence_tagged[i] != ']':
                _sentence.append(sentence_tagged[i])
                i+=1
                if i == len(sentence_tagged):
                    break
        else:
            _sentence.append(sentence_tagged[i].split('#')[0])
            i+=1
    sentence = ' '.join(_sentence)
    phrases = set(re.findall(r"\[\s[\w\d\#\s]+\]",sentence))
    sentence = sentence.replace('[','')
...
def tokenizeUNS(content, token_min_len=TOKEN_MIN_LEN, token_max_len=TOKEN_MAX_LEN, lower=True):
    sentence =  [utils.to_unicode(token) for token in utils.tokenize(content, lower=lower, errors='ignore')
               if token_min_len <= len(token) <= token_max_len and not token.startswith('_')]
    sentence_ = filter(lambda i: False if i in '[] ' else True, sentence)
    sentence_ = list(sentence_)
    ...
    doc = nlp2(sentence)
    phrases = [ent for ent in doc.ents if len(ent)>1]
    for phrase in phrases:
        phrase = str(phrase)
        sentence = re.sub(phrase, ' [ '+phrase+' ] ', sentence)
    phrases = set(re.findall(r"\[\s[\w\d\#\s]+\]",sentence))
...
def tokenizeUNSEM(content, token_min_len=TOKEN_MIN_LEN, token_max_len=TOKEN_MAX_LEN, lower=True):
    sentence =  [utils.to_unicode(token) for token in utils.tokenize(content, lower=lower, errors='ignore')
               if token_min_len <= len(token) <= token_max_len and not token.startswith('_')]
    sentence_ = filter(lambda i: False if i in '[] ' else True, sentence)
    sentence_ = list(sentence_)
...
def tokenizeUNSDEP(content, token_min_len=TOKEN_MIN_LEN, token_max_len=TOKEN_MAX_LEN, lower=True):
    sentence =  [utils.to_unicode(token) for token in utils.tokenize(content, lower=lower, errors='ignore')
               if token_min_len <= len(token) <= token_max_len and not token.startswith('_')]
    sentence_ = filter(lambda i: False if i in '[] ' else True, sentence)
    sentence_ = list(sentence_)
...
class WikiCorpus(TextCorpus):
    ...
    * <LANG>wiki-latest-pages-articles.xml.bz2
    ...
    def __init__(self, fname, processes=None, lemmatize=utils.has_pattern(), dictionary=None,
                 filter_namespaces=('0',), tokenizer_func='', article_min_tokens=ARTICLE_MIN_WORDS,
        ...
        if tokenizer_func == '':
            ...
            self.tokenizer_func = tokenizeUNS
        elif tokenizer_func == 'UNSEM':
            self.tokenizer_func = tokenizeUNSEM
        elif tokenizer_func == 'UNSDEP':
            self.tokenizer_func = tokenizeUNSDEP
