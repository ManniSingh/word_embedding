    ...
    for sent in sentences:
        ...
        while i<end:
            ...
            if sent[i] == '[':
                ...
                #i is ']' now 
                if i<end:
                    assert sent[i]==']',"Target mismatch error"
                #print("Phrase end:",i)
            else:
                # X exclusive case
                i+=1
            ...
            plen = i-t_start-1
            if not targets or plen < 2 or len(targets) > 10:
            # Strict N-gram phrase where N>=2.
            #if len(targets) != 2:
                i+=1
                continue
            ...
            #while j < (i + c.window): # incase of no random window reduction
                if r<end:
                    ...
                    if right_con == '[':
                        ...
                        right_phrase = list()
                        while sent[r] != ']':
                            #if sent[r] in vlookup and sent[r][0].isupper() and sent[r][1:].islower(): #SP (capital words only)
                            rcon = sent[r] + '#'+str(pos)
                            #pos+=1
                            if rcon in vlookup :
                                right_phrase.append(vlookup[rcon].index)
                            r+=1
                            if r>=end :
                                break
                        if right_phrase and len(right_phrase)<=10:
                            ...
                            for target in targets:
                                for con in context:
                                    ...
                                    #print("target:",id2word[target],"context:",id2word[con])
                                    effective_words += 1 
                ...
                if l>=0:
                    ...
                    if left_con == ']':
                        ...
                        while sent[l] != '[':
                            lcon = sent[l] + '#'+str(-pos)
                            #pos+=1
                            #if sent[l] in vlookup and sent[l][0].isupper() and sent[l][1:].islower(): #SP (capital words only)
                            if lcon in vlookup :
                                left_phrase.append(vlookup[lcon].index)
                            l-=1
                            if l<0:
                                break
                        if left_phrase and len(left_phrase)<=10:
                        ...
                        lcon = left_con + '#'+str(-pos)
                        #pos+=1
                        context = vlookup[lcon].index
                        for target in targets:
                            c.indexes[effective_words] = target
                            c.contexts[effective_words] = context
                            #print("target:",id2word[target],"context:",id2word[context])
