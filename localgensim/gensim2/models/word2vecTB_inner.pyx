...
def train_batch_sg(model, sentences, alpha, _work, compute_loss):
    ...
    for sent in sentences:
        ...
        while i<end:
            ...
            if i>=end:
                break
            if not targets or len(targets) < 2 or len(targets) > 3:
            # Strict N-gram phrase where N>=2.
            #if len(targets) != 2:
                i+=1
                continue
            _targets.append(targets[0]+'#L')
            t_ = 1
            while t_<len(targets):
                if t_ == len(targets)-1:
                    _targets.append(targets[t_]+'#R')
                    t_+=1
                    continue
                _targets.append(targets[t_]+'#M')
                #_targets.append(targets[t_]+'#L')
                #_targets.append(targets[t_]+'#R')
                t_+=1
            #print(_targets)
            for word in _targets:
                if word in vlookup:
                    targets_.append(vlookup[word].index)
            #print(targets_)
            #print(i) ###debug
            #print(end) ###debug
            #time.sleep(1) ####debug
                ...
                if r<end:
                    ...
                    if right_con == '[':
                        ...
                        while sent[r] != ']':
                            ...
                            if r>=end :
                                break
                        if right_phrase and len(right_phrase)<=3:
                ...
                if l>=0:
                    ...
                    if left_con == ']':
                        ...
                        while sent[l] != '[':
                            ...
                            if l<0:
                                break
                        if left_phrase and len(left_phrase)<=3:
